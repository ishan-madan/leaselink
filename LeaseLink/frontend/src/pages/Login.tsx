...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // get form data -> grab email and password from form
    const formData = new FormData(e.currentTarget);
    const email = formData.get("email") as string;
    const password = formData.get("password") as string;

    // try catch block to log in
    try {
      // call auth login function to log user in
      toast.loading("Signing In...", {id:"login"});
      await auth?.login(email, password);
      toast.success("Logged In", {id:"login"});

      // automatically navigate to incidents page after logging in
      navigate('/incidents');
    } catch (error) {
      console.log(error);
